...
public class Main {
    ...
    private static class GoGameMain extends JFrame implements ActionListener{
        ...
        public GoGameMain() 
        {
            ...
            this.add(boardPanel);
            this.setTitle("Go 9x9 Game  Made By GaoTao");
        ...
        private void setMenu() 
	    {
			...
			String[][] menuCommands = { { "Game", "Begin", "PlayWithPC", "Finish", "", "Exit" }};
            ...
            for(i = 4; i < boardInt.length-4; i++)
            {
                for(j = 4; j<boardInt[i].length-4; j++)
                {
                    if(boardInt[i][j]==0 && (checkAvailable(i,j,userFlag) || !checkEat(i,j,userFlag).isEmpty()))
                    {
                        ...
                        stones[steps][1]=j;
                        steps++;
                        for(i1 = 4; i1 < boardInt.length-4; i1++)
                        {
                            for(j1 = 4; j1 < boardInt[i1].length-4; j1++)
                            {
                                if(boardInt[i1][j1]==0 && (checkAvailable(i1,j1,-userFlag) || !checkEat(i1,j1,-userFlag).isEmpty()))
                                {
                                   stones[steps][0]=i1;
                                   stones[steps][1]=j1;
                                   steps++;
                                   for(i2 = 4; i2 <boardInt.length-4; i2++)
                                   {
                                       for(j2 = 4;j2 < boardInt[i2].length-4; j2++)
                                       {
                                           if(boardInt[i2][j2]==0 && (checkAvailable(i2,j2,userFlag) || !checkEat(i2,j2,userFlag).isEmpty()))
                                           {
                                                stones[steps][0]=i2;
                                                stones[steps][1]=j2;
                                                steps++;
                                                monteCarloMethod mCM = new monteCarloMethod();
                                                mark2[i2][j2] = mCM.monteCarlomethod(stones,steps);
                                                steps--;
                                                boardInt[i2][j2]=0;
                                                if(mark2[i2][j2]>maxMark)
                                                    maxMark = mark2[i2][j2];                                              
                                           }
                                       }
                                   }
                                   ...
                                   boardInt[i1][j1]=0;
                                   mark1[i1][j1] = maxMark;
                                   if(mark1[i1][j1]<miniMark){
                                       miniMark = mark1[i1][j1];
                                }
                                }
                            }
                        }
